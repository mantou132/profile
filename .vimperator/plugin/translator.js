...
FGgTranslator = function (window, document){
// ==FireGesturesScript==
// @name           FGgTranslator
// @author         Crab
...
// ==/FireGesturesScript==
	if(!window.vimpFGgTranslator){
		window.vimpFGgTranslator = {
			offset: {
			...
			init: function(event){
				...
				this.boxElements.box.style.right  = '0px';
				this.boxElements.box.style.bottom = '0px';
			...
			xpPref:function(value){
				var pref = 'FireGestures.vimpFGgTranslator.optionJSON';
			...
			getSelection: function(event){
				// var view = event.view,
				// 	selection = view.getSelection(event),
				// 	txt = '';
...
mappings.addUserMap(
    ...
    '翻译',
    function (count) {
    	// content.window.console.log(content);
    	FGgTranslator(content.window, content.window.document);
    }, 
    {
        count: true,
    }
